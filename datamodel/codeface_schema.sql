...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `codeface`.`view_developer` (`developerName` INT, `projectId` INT, `component` INT, `isOpen` INT, `'issueAssigned'` INT, `'avgTime'` INT, `'numCommentPosted'` INT, `'numAttachmentPosted'` INT, `'positiveReviews'` INT, `'sizeAttachmentPosted'` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `codeface`.`view_reality_check` (`'projectId'` INT, `'TP'` INT, `'FP'` INT, `'FN'` INT);

-- -----------------------------------------------------
-- Placeholder table for view `codeface`.`view_developer_statistic`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `codeface`.`view_developer_statistic` (`projectId` INT, `assignedTo` INT, `isOpen` INT, `'spentTime'` INT);
...
SELECT r.developerName, r.projectId, r.component, r.isOpen, sum(r.issueAssigned) as 'issueAssigned', sum(r.avgTime) as 'avgTime', sum(r.numCommentPosted) as 'numCommentPosted',
...
GROUP BY
		r.developerName, r.projectId, r.component, r.isOpen;
...
FROM
        ...
        JOIN view_developer df ON (b.projectId = df.projectId AND b.component = df.component AND d.developerName = df.developerName)
        JOIN (SELECT d.projectId, d.component, floor(avg(d.avgTime)) as 'bugAvgETA'
				FROM view_developer d WHERE isOpen = 0 GROUP BY d.projectId, d.component) dr ON (b.projectId = dr.projectId AND b.component = dr.component)
WHERE
...
SELECT
		TTP.projectId as 'projectId',
        TTP.NUM as 'TP',  TFP.NUM as 'FP', TFN.NUM as 'FN'
FROM
		(SELECT i1.projectId, COUNT(a1.issueId) AS NUM
				FROM issue_data i1 LEFT JOIN issue_assignment a1 ON (i1.issueId = a1.issueId AND
				i1.projectId = a1.projectId AND i1.realAssignee = a1.developerName)
                WHERE i1.realAssignee <> 'NULL'
                GROUP BY i1.projectId) AS TTP,
		(SELECT i2.projectId, COUNT(a2.issueId) AS NUM
				FROM issue_data i2 LEFT JOIN issue_assignment a2 ON (i2.issueId = a2.issueId AND
				i2.projectId = a2.projectId AND i2.realAssignee <> a2.developerName)
                WHERE i2.realAssignee <> 'NULL'
                GROUP BY i2.projectId) AS TFP,
		(SELECT i3.projectId, COUNT(i3.issueId) AS NUM
				FROM issue_data i3 LEFT JOIN issue_assignment a3 ON (i3.issueId = a3.issueId AND
				i3.projectId = a3.projectId AND i3.realAssignee <> a3.developerName)
                WHERE i3.realAssignee <> 'NULL'
                GROUP BY i3.projectId) AS TFN
WHERE
        TTP.projectId = TFP.projectId AND
        TTP.projectId = TFN.projectId
GROUP BY
		TTP.projectId
ORDER BY
		TTP.projectId ASC;
...
-- -----------------------------------------------------
-- View `codeface`.`view_developer_statistic`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `codeface`.`view_developer_statistic` ;
DROP TABLE IF EXISTS `codeface`.`view_developer_statistic`;
USE `codeface`;
CREATE OR REPLACE VIEW `view_developer_statistic` AS
SELECT i.projectId, i.assignedTo, i.isOpen, sum(i.spentTime) as 'spentTime'
FROM
		issue_data i
GROUP BY
		i.projectId, i.assignedTo, i.isOpen;
USE `codeface`;
